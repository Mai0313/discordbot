...
賭場遊戲的 final response embed 會在回合結算三分鐘後自動刪除，餘額不足而拒絕開局的回覆也會在三分鐘後清掉，避免頻道被小遊戲洗版。

| Slash command       | 玩法                                                                                                                   |
| ------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `/dice <下注>`      | 三顆骰子 vs 三顆骰子，總和大者贏（平手退錢）。                                                                         |
| `/blackjack <下注>` | 標準 21 點，附 Hit / Stand button；天生 Blackjack 賠 1.5 倍；玩家每次 hit 莊家會用看得到的 dealer card 給帶刺的 hint。 |
