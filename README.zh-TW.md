...
貼上 Threads.net 連結，機器人會自動展開貼文 — 顯示文字內容、圖片、互動數據，並下載附帶的影片。若連結指向某則回覆，機器人會把整條回覆鏈一併展開，原始貼文與中間每一層回覆都會出現在同一則訊息裡，並用灰階漸層色帶（由淡到深）區分層級。只有使用者貼的那一篇會把影片下載並附上來，上層回覆若有影片則改用點擊連結提示，避免多層附件混在一起無法分辨。成功展開後，貼出連結的人會獲得點數（詳見下方 *點數系統與賭場小遊戲*）。
...
- 成功下載時，呼叫者會獲得點數（詳見下方 *點數系統與賭場小遊戲*）
...
- **跟機器人聊天** — 每次 AI 串流回覆會以 `total_tokens` (input + output) 為點數獎勵，實際數字會顯示在回覆 footer。
- **`/download_video`** — 成功下載後付給呼叫者 `10 + round(file_size_mb)` 點，上限 100；獎勵會直接 append 到「下載成功」訊息。
- **Threads 連結** — 機器人成功展開 Threads.net 連結後，**貼連結的人**獲得 `10 + 影片數 × 5 + 圖片數 × 2` 點。同一位使用者貼同一個 URL 在 1 小時內只算一次，避免複製貼上刷分。

**花用點數：** 賭場遊戲會先扣下注額，結算時再依結果回補。莊家是個 AI，開局會嘴一下注金額，結算時會依結果嘴或誇玩家。
...
- `/balance` — 查看自己的餘額。
- `/leaderboard` — 機器人所有伺服器的全域 Top 10。
...
### Slash Command 綁定的 Guild

`/balance`、`/leaderboard`、`/give`、`/dice`、`/blackjack` 這 5 個指令會被綁定到 `src/discordbot/typings/config.py:FAST_SYNC_GUILD_IDS` 列出的 guild ID（這樣可以即時生效，否則 Discord 全域 slash-command sync 要等最多 1 小時）。預設清單寫的是 maintainer 平常用的兩個 server — **自己 host 的話請編輯這個 list 換成你自己的 guild ID, 否則這 5 個指令在你的 server 不會出現**。其他指令（`@bot` 聊天、`/download_video`、`/maple_*`、`/help`、`/ping`）走全域 sync，不受影響。
