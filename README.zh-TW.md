...
- **Threads 解析**：貼上 Threads.net 或 Threads.com URL，bot 會展開貼文、media 與 reply chain；tag bot 並附上連結的話，它還會讀過貼文再回答。
- **抖音解析**：貼上抖音連結，bot 會直接把影片（或圖文貼文的圖片）傳到頻道；tag bot 並附上連結的話，它還會看過影片再回答。
...
| _Threads URL_                                                    | 自動展開 Threads 貼文與 media；被 tag 時還會讀過貼文再回答。            |
| _抖音 URL_                                                       | 自動傳回影片或圖片；被 tag 時還會看過影片再回答。                       |
