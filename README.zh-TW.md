...
- **AI chat**：在 server tag bot 或傳送 DM。它可以回答問題、總結近期聊天、檢查支援的附件、生成或編輯圖片、生成短影片、以接續 reply 訊息延續長回覆，並在可用時使用 model-provided web tools。它還會在背景慢慢累積對你個人偏好的長期記憶（跨伺服器、僅自己可見），可用 `/memory show` 查看（`/memory clear` 暫時停用）。
...
- **賭場遊戲**：多人 `/games blackjack` 與 `/games dragon_gate` lobby。Blackjack 莊家改為賭場系統 (deterministic H17)，bot 本身會以玩家身份入桌並由獨立 AI 策略決策，`/casino` 與 `/pocat` 分別顯示賭場帳本與 bot 玩家錢包。單人 `/games fishing` 則是買釣具拋竿、回收歡樂豆的 sink 玩法，魚分 N 到 UR 稀有度並有最大單筆漁獲排行榜。
...
| `/memory show\|clear`                                            | 私密查看或清除 bot 對你記住的內容（show 可加「詳細」選項查看詳細記錄；clear 暫時停用）。 |
...
個人長期記憶預設開啟；設定 `MEMORY_ENABLED=false` 可同時停用記憶注入與背景抽取。`/memory clear` 預設暫停；設定 `MEMORY_CLEAR_ENABLED=true` 才會真正清除記憶。
