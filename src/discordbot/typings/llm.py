...
class LLMConfig(BaseSettings):
    ...
    Attributes:
        ...
        inline_music_enabled: Kill-switch for inline generated music on QA replies; when false
            the answer model's `<music>` marker is still stripped but no clip is generated.
        inline_video_enabled: Kill-switch for inline generated video on QA replies; when false
            the answer model's `<video>` marker is still stripped but no clip is generated.
    ...
    def music_available(self) -> bool:
        ...
        Interactions API); without it the render would just fail, so the QA `<music>` marker is
    ...
    def video_available(self) -> bool:
        ...
        just fail, so the QA `<video>` marker is only advertised when both the kill-switch is on
