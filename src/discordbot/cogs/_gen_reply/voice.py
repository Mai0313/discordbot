"""Optional spoken-reply synthesis: turn the marked span of a reply into a voice clip.

The answer model wraps the exact part of its reply it wants read aloud in `<voice>...</voice>`
(parsed in `markup.py`). `ResponseStreamer` keeps that span in the visible text but feeds only
it to `VoiceSynthesizer`, which renders it to audio and edits the clip onto the already-sent
