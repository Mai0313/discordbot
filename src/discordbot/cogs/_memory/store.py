...
also carry ``tone.md``: a short, persona-independent note of how that user
wants the bot to sound, written by a dedicated single-call updater and injected
on every reply for the message author (no selection phase), kept short by its
prompt rather than a compaction pass. The live files
...
def write_tone(scope: str, content: str) -> None:
    ...
    Shortness is enforced by the tone-update prompt, not a compaction pass; the
    ...
    generation: the note is best-effort and the next update repairs a bad write.
    """
    _scope_dir(scope=scope).mkdir(parents=True, exist_ok=True)
    rendered = content.strip()
