...
_HELP_CONTENT = {
    ...
    Locale.zh_TW: {
        ...
        "stocks": (
            "**模擬股市** — `/stock`（`/股票`）\n"
            "開啟一則公開 simulated market message，內含 DB-managed virtual companies；選股、資金、部位、新聞、買入/做空 operation dropdown、quantity modal、settlement result、部位摘要、近期交易紀錄與 7D chart 都會在同一則公開 message 內 edit 切換。news 會定期刷新，有 AI 設定時走 AI，否則走 deterministic fallback。公開 stock message 180 秒無互動後自動清理，只有發起 `/stock` 的 user 可以操作 controls。"
        ),
        "vip": (
            "**VIP** — `/vip`\n"
