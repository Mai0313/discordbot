...
_HELP_CONTENT = {
    ...
    Locale.zh_TW: {
        ...
        "stocks": (
            "**模擬股市** — `/stock`（`/股票`）\n"
            "開啟公開 simulated market message，內含多家 virtual company；選股後會開 private detail panel 顯示資金、部位、新聞、買入/做空、settlement result、部位摘要與近期交易紀錄。news 會定期刷新，有 AI 設定時走 AI，否則走 deterministic fallback。公開 market message 180 秒無互動後自動清理，只有開啟面板的 user 可以操作 controls。"
        ),
        "vip": (
            "**VIP** — `/vip`\n"
