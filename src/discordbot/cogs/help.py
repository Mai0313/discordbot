...
_HELP_CONTENT = {
    "default": {
        ...
        "games": (
            "**Games**\n"
            "`/games blackjack` starts a Blackjack table; set `bet` to `0` for all in. `/games dragon_gate` starts a Dragon Gate jackpot table."
        ),
    ...
    Locale.zh_TW: {
        ...
        "games": (
            "**小遊戲**\n`/games blackjack` 開 21 點桌，`bet` 寫 `0` 就是 all in。`/games dragon_gate` 開射龍門 jackpot 桌。"
        ),
    ...
    Locale.ja: {
        ...
        "games": (
            "**ゲーム**\n"
            "`/games blackjack` は `bet` を `0` にすると all in の Blackjack table、`/games dragon_gate` は Dragon Gate jackpot table を開きます。"
        ),
