...
_HELP_CONTENT = {
    "default": {
        ...
        "stocks": (
            ...
            "Open the public simulated market message with DB-managed virtual companies. Selecting a stock opens a private detail panel for balance, positions, news, buy/short operations, settlement results, position summaries, and recent trade history. News refreshes periodically with AI when configured and deterministic fallback otherwise. The public market message cleans itself up after 180 idle seconds. Only the user who opened the panel can operate its controls."
        ),
        "vip": (
            "**VIP** — `/vip`\n"
    ...
    Locale.zh_TW: {
        ...
        "stocks": (
            "**模擬股市** — `/stock`（`/股票`）\n"
            "開啟公開 simulated market message，內含 DB-managed virtual companies；選股後會開 private detail panel 顯示資金、部位、新聞、買入/做空、settlement result、部位摘要與近期交易紀錄。news 會定期刷新，有 AI 設定時走 AI，否則走 deterministic fallback。公開 market message 180 秒無互動後自動清理，只有開啟面板的 user 可以操作 controls。"
        ),
        "vip": (
            "**VIP** — `/vip`\n"
    ...
    Locale.ja: {
        ...
        "stocks": (
            "**シミュレーション株式市場** — `/stock`\n"
            "DB-managed virtual companies を含む公開 simulated market message を開き、stock selection 後は private detail panel で balance、positions、news、buy/short operations、settlement results、position summaries、recent trade history を表示します。news は AI 設定があれば定期生成され、なければ deterministic fallback を使います。公開 message は180秒 idle で自動削除され、panel を開いた user だけが controls を操作できます。"
        ),
        "vip": (
            "**VIP** — `/vip`\n"
