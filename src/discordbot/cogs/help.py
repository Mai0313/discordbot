...
_HELP_CONTENT = {
    "default": {
        ...
        "games": (
            ...
            "the wager and draws one card before auto-standing. Split is only available on "
            "a strict same-rank pair; **Double after Split is disabled** (No DAS). Splitting "
            "Aces takes one card per hand and 21 on a split hand pays 1:1 (not natural 3:2). "
            "Late Surrender refunds half the wager.\n"
            "When the dealer up-card is **A** the round pauses for an insurance side bet "
            "(½ of the original wager, pays 2:1 if the dealer peeks Blackjack). When the "
            "dealer up-card is **A or 10-value** the dealer peeks the hole card; a natural "
    ...
    Locale.zh_TW: {
        ...
        "games": (
            ...
            "Double 加倍下注後只再抽一張即強制停牌；Split 僅同 rank pair 可分，"
            "**Split 後禁止 Double**（No DAS），Split Aces 兩手各只能再拿一張，"
            "且 21 點算一般 win（不是 natural Blackjack 3:2）；Late Surrender 退回一半本金\n"
            "莊家明牌 **A** 時會先進入保險階段（保險注 = 原注一半，莊家 BJ 賠 2:1）；"
    ...
    Locale.ja: {
        ...
        "games": (
            ...
            "Double はベットを倍にして1枚だけ引いて自動 stand。Split は同 rank pair のみ可、"
            "**Split 後は Double 不可** (No DAS)、Split Aces は各手1枚だけで finished、"
            "21 になっても natural BJ 扱いではなく 1:1。Late Surrender は半額返却。\n"
            "dealer up-card が **A** のとき insurance phase が入り (原 bet の半額、dealer BJ なら 2:1)、"
