...
_HELP_CONTENT = {
    "default": {
        ...
        "stocks": (
            ...
            "`/stock` opens the market board UI for prices, market-context news, positions, charts, supply-capped buy and short entries, plus sell and cover exits."
        ),
        "vip": (
            "**VIP**\n"
        ...
        "games": (
            "**Games**\n"
            "`/games blackjack` starts a Blackjack table, with `all_in` for full-balance stakes. `/games dragon_gate` starts a Dragon Gate jackpot table."
        ),
    ...
    Locale.zh_TW: {
        ...
        "stocks": (
            "**模擬股市**\n"
            "`/stock` 開啟 market board UI，可以看價格、參考 market context 的 news、position、chart，也能建立受 supply cap 限制的 buy/short，或執行 sell/cover 出場。"
        ),
        "vip": ("**VIP**\n`/vip` 購買或查看 VIP 狀態，VIP 會加成 check-in 和 Blackjack reward。"),
        "games": (
            "**小遊戲**\n`/games blackjack` 開 21 點桌，可以用 `all_in` 直接押目前餘額。`/games dragon_gate` 開射龍門 jackpot 桌。"
        ),
    ...
    Locale.ja: {
        ...
        "stocks": (
            "**シミュレーション株式市場**\n"
            "`/stock` で market board UI を開き、price、market context 付き news、position、chart、supply cap 付きの buy/short entry、sell/cover exit を扱えます。"
        ),
        "vip": (
            "**VIP**\n"
        ...
        "games": (
            "**ゲーム**\n"
            "`/games blackjack` は `all_in` で全残高 stake の Blackjack table、`/games dragon_gate` は Dragon Gate jackpot table を開きます。"
        ),
