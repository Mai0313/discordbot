...
- **抽獎系統**：多平台抽獎功能，支援 Discord「按鈕報名」或 YouTube 聊天室整合（不再使用表情反應），可設定每次抽出人數並支援重新建立。
    - 實作說明：報名/取消按鈕以 `nextcord.ui.Button` 的子類別形式實現（`JoinLotteryButton`、`CancelJoinLotteryButton`），提升可維護性並利於未來 persistent view 支援。
- **圖像生成**：框架已準備（預留實現）
...
| `/lottery` | 抽獎主指令 | 下拉選擇方式；按鈕：🎉 報名、🚫 取消、✅ 開始（主持人）、📊 狀態（僅自己可見）、🔄 重新建立；建立訊息會自動更新參與者名單；可設定每次抽出人數 |
...
- **報名模式**：Discord 按鈕報名或 YouTube 聊天室關鍵字報名（避免跨平台重複）
- **控制按鈕**：🎉 報名、🚫 取消、✅ 開始（僅主持人）、📊 狀態（僅本人可見）、🔄 重新建立
- **即時狀態**：按 📊 取得僅自己可見的活動狀態
- **自動更新訊息**：建立訊息會自動更新參與者名單（使用者報名/取消時即時反映）
- **訊息綁定**：按鈕互動會綁定建立訊息，機器人可藉此辨識對應的抽獎（完全不使用表情反應）
- **YouTube 模式名單抓取**：在 YouTube 模式下，主持人按下 ✅ 開始 時，機器人會依設定關鍵字從聊天室抓取參與者
