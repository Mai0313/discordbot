...
**Spending Points:** casino games open a lobby first. The owner can start alone or wait for other players to join; only the owner can start the table. Blackjack bets are validated when a player joins and refreshed when the table starts, then the signed result is applied only when the table resolves. If the Blackjack owner enters more than their current balance, the lobby table stake is clamped to the owner's actual all-in amount, so later players default to matching that stake instead of the original oversized request; only a zero or negative balance rejects the player. 射龍門 runs over a **global jackpot pool shared across every table** (one row in `jackpot_pool`, seeded with 100,000 on the house and automatically replenished to 100,000 whenever the pool is won out). The ante is fixed at 5,000 (paid into the pool on table start), the minimum bet is 10,000, the maximum bet is the entire pool, and each bet settles into the player row and the pool the instant it lands. Dragon Gate losses clamp at balance 0; the jackpot receives only the amount actually debited, and any player who hits 0 automatically leaves while the rest of the table continues. Adjacent non-pair pillars have no gate and are redealt without a bet. Any seated player can leave mid-table; if their running delta is positive at leave / timeout, the surplus is refunded into the pool ("逆贏不拿"). The table ends when the whole pool is won, every player has left or hit 0, or no one interacts for 180 seconds. The dealer is an AI that taunts the table, shows a thinking state while deciding Blackjack hit / stand after all players finish, and reacts to the result with one short line. The dealer's display name in the embed (and in message history seen by `gen_reply`) is the bot's own Discord display name, so it shows up as a familiar identity rather than a generic "dealer" label. Final game embeds show each player's round delta, post-settlement balance, and the Blackjack dealer decision path; `/house` carries the Blackjack dealer's ledger balance (Dragon Gate's counterparty is the jackpot pool, not the house).
...
Interactive game and lobby messages use a 180-second idle timeout, meaning the timer is based on no component interactions rather than wall-clock time from creation. Once a public response is no longer interactive, it is automatically deleted three minutes after it is sent or settled. This includes final casino round embeds, rejected zero-balance bets, `/leaderboard`, `/loss_leaderboard`, `/house`, `/give` result embeds, and successful `/admin refund_tax` / `/admin collect_tax` adjustment embeds. `/balance`, `/borrow`, `/repay`, `/checkin`, `/vip`, and admin error replies are private to the caller. Game response message IDs are stored locally so a bot restart can delete stale in-progress or already-settled game embeds on the next startup.

| Slash command      | Game                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/blackjack <bet>` | Multiplayer 21 lobby with Join / Leave / Start buttons. Player actions include **Hit / Stand / Double Down / Split / Surrender**, plus Insurance controls only while the dealer up-card Ace side bet is available and the player still has uncommitted balance. After every hand stands or busts the table shows the AI dealer thinking, then the dealer decides hit / stand for the house (Soft 17 is left up to the model).                                                |
| `/dragon_gate`     | Multiplayer 射龍門 lobby over the **global jackpot pool** (cross-table cumulative, system-reseeded to 100,000 when won out). Fixed 5,000 ante into the pool, min bet 10,000, max bet = current pool, every bet settles instantly, adjacent non-pair pillars redeal without a bet, per-player Leave button with 逆贏不拿 refund, 180s idle timeout. Gate-win pays 1× from the pool; losses charge up to the player's current balance, then a zero-balance player auto-leaves. |

**Blackjack actions:** Double Down doubles the current bet, draws exactly one card, and auto-stands. Split is only available on a strict same-rank pair; **Double after Split is disabled** (No DAS), and splitting Aces takes one card per half and turns a 21 into a regular 1× win (not a natural Blackjack 1.5×). Late Surrender returns half the wager and is closed once the dealer peeks Blackjack.

**Blackjack early settlement & peek:** `Blackjack` means the first two cards are an ace plus a 10-value card; a natural pays 1.5×. The dealer is dealt one hidden hole card and one visible up-card; during player actions the table renders this as `🂠 <up-card>` so the hidden card is explicit without revealing its value. When the dealer up-card is an Ace the round pauses for an Insurance side bet (½ of the original wager, requires uncommitted balance, pays 2:1 if the dealer peeks Blackjack), and the Insurance Yes / No controls are only shown during that phase. When the up-card is any 10-value card the dealer peeks silently. A peek Blackjack settles the table immediately, except a player who also has a natural pushes. A regular 21 reached with more cards is not a natural Blackjack and does not skip the action phase.

**Managing Points:**

- `/balance` — privately show your current balance, loan principal (if any), and VIP status.
...
| `/balance`                        | Privately show your current Points balance, loan, and VIP status (cross-server)       |
...
- **Game Cleanup Database**: Pending public game and economy response cleanup stores only Discord channel IDs and message IDs in `data/game_cleanup.db` so stale cleanup targets can be deleted after a bot restart.
