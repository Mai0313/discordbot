...
**Blackjack early settlement & peek:** `Blackjack` means the first two cards are an ace plus a 10-value card; a natural pays 1.5×. When the dealer up-card is an Ace the round pauses for an Insurance side bet (½ of the original wager, requires uncommitted balance, pays 2:1 if the dealer peeks Blackjack), and the Insurance Yes / No controls are only shown during that phase. When the up-card is any 10-value card the dealer peeks silently. A peek Blackjack settles the table immediately, except a player who also has a natural pushes. A regular 21 reached with more cards is not a natural Blackjack and does not skip the action phase.
