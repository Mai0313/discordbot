...
- **AI chat**: mention the bot in a server or send a DM. It can answer questions, summarize recent chat, inspect supported attachments, generate or edit images, generate short videos, continue long replies as follow-up reply messages, and use model-provided web tools when available. It also builds a private per-user long-term memory of your preferences in the background, keeps a short note on the tone you like it to use and adapts how it talks to you, and is manageable with `/memory show`, `/memory clear`, and `/memory regenerate`.
