...
- Casino settlement applies one signed result after play. Validate or clamp bets before play, then settle once through the settlement helpers. Player-side casino losses clamp at balance 0; dealer / house ledgers may still go negative. Cross-file casino ledger mirroring has the same SQLite limitation as jackpot settlement: ordinary pre-commit errors roll both sessions back, but a hard crash between database-file commits is not cross-file atomic.
