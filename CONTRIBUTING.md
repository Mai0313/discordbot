...
│   │   ├── cleanup.py       # Timed deletion helper for casino response messages
...
- **Game flow**: `/dice` and `/blackjack` (in `cogs/games.py`) withdraw the bet up-front via `place_bet(...)`, which auto-clamps over-bets to all-in and rejects only zero-balance users. Finished rounds go through `cogs/_games/settlement.py:settle_wager(...)`, which credits `bet + player_delta` back to the player and mirrors `-player_delta` into the bot's house-ledger row via `house_settle(...)`. Final casino response messages are scheduled for deletion 180 seconds after settlement through `cogs/_games/cleanup.py`; rejected zero-balance bets are also deleted after 180 seconds. `BlackjackView` (`cogs/_games/views.py`) drives Hit/Stand buttons, auto-stands on timeout, and uses a lock plus `_settled` flag so concurrent finalization cannot pay the same hand twice. `DealerAI` (`cogs/_games/dealer.py`) wraps the fast model for `taunt_bet` / `settle` / `hint` banter; every entry point falls back to a hard-coded line on LLM failure so the round always resolves. The "dealer" name shown in embeds and the house-ledger row is taken from `bot.user.display_name`.
