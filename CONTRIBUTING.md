...
- **Economy**: Per-user point balances live in a separate SQLite (`data/economy.db`) managed by `cogs/_economy/database.py`. The schema is keyed by Discord `user_id` only — **no `guild_id`**, so balances and the `/leaderboard` are intentionally cross-server. Sync ORM helpers (`_add_balance_sync`, `_settle_game_sync`, `_house_settle_sync`, `_transfer_sync`, `_top_n_sync`, `_get_account_sync`) are wrapped with `asyncio.to_thread` to keep the event loop responsive. Only `gen_reply.py` pays points (one per token) — Threads, video, template, and maplestory deliberately do not pay.
- **Game flow**: `/dice` and `/blackjack` (in `cogs/games.py`) deduct the bet up-front via `settle_game(delta=-bet)`, run pure-rule helpers from `cogs/_games/{dice,blackjack}.py`, then credit `bet + delta` back when the round resolves. Every player settlement is mirrored into the bot's own `UserAccount` row via `house_settle(delta=-player_delta)` so the casino's running P&L is always visible via `/house`; unlike `settle_game`, `house_settle` does not clamp at zero (the bot has effectively infinite funds, so its balance can go negative). `BlackjackView` (`cogs/_games/views.py`) drives Hit/Stand buttons and auto-stands on timeout. `DealerAI` (`cogs/_games/dealer.py`) wraps the fast model for `taunt_bet` / `settle` / `hint` banter; every entry point falls back to a hard-coded line on LLM failure so the round always resolves. The "dealer" name shown in embeds (description prefix, field titles, house-ledger row) is taken from `bot.user.display_name` so the bot's own messages look like a familiar identity rather than a generic "dealer".
...
- **Economy DB** (`tests/test_economy.py`): per-test isolated SQLite via monkeypatched `_engine`; covers add / settle clamping / transfer atomicity / leaderboard ordering with exclude / `house_settle` allowing negative balances / `get_account` returning gross flows
