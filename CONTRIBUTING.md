...
│   └── video.py             # /download_video slash command; awards points on successful download (file delivered as a separate followup so the reward suffix isn't dropped)
├── typings/                 # Pydantic configuration & shared models
│   ├── config.py            # DiscordConfig (DISCORD_BOT_TOKEN, DISCORD_TEST_SERVER_ID) + FAST_SYNC_GUILD_IDS module-level list
...
- **Fast-sync guild pinning**: `typings/config.py` exposes a module-level `FAST_SYNC_GUILD_IDS: list[int]`. Slash commands that pass `guild_ids=FAST_SYNC_GUILD_IDS` are registered only against those guilds (instant rollout, but invisible elsewhere). Currently used by the five economy / casino commands (`/balance`, `/leaderboard`, `/give`, `/dice`, `/blackjack`). Edit the constant to retarget; it lives outside `DiscordConfig` because slash-command decorators are evaluated at import time and can't depend on a `BaseSettings` instance.
