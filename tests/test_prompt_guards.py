...
two DISCUSSION paths only, Threads and Facebook, each of which hands the model thousands of
characters written by strangers: a Douyin caption, a Bilibili title, a page fetched through
`urlContext`, a transcript and an uploaded file all reach the model undefused. The prompt rule
telling the model never to echo such a tag verbatim is what covers them, so it is the load-bearing
half of that defence rather than advice.
