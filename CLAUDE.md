...
- `UserAccount` has no `guild_id`. Identity, VIP, check-ins, admin flags, central banker flags, and leaderboard visibility are cross-server by design. Spendable balances and gross totals live in `user_wallet`; long-term loans live in `loan_proposal` and `loan_contract`; daily casino counters live in `casino_account`.
...
- Stock helpers store player-issued stock in `stock_profile`, `stock_holding`, and `stock_event`. One issuer can have one profile; issuing is gated by spendable `user_wallet.balance`, not net worth; buyers purchase treasury shares directly from the issuer; dividends are manual cash distributions to sold-share holders.
...
- Economy visibility follows the event privacy boundary. Public embeds are for shared social, market, or settlement events that other players can react to: `/give`, leaderboards, `/house`, successful admin adjustments, loan requests and decisions, `/credit repay`, `/credit call`, `/central_bank repay`, `/central_bank call`, `/stock issue`, `/stock buy`, `/stock dividend`, and `/stock info`. Public economy embeds schedule cleanup after send.
- Ephemeral economy embeds are for personal state and validation or permission failures: `/balance`, `/portfolio`, `/checkin`, `/vip`, `/credit status`, `/central_bank status`, insufficient-balance failures, missing-loan failures, and permission errors.
