...
**`data/messages.db` ONLY records human messages — NEVER bot messages.** Both `MessageLogger.log` and `LogMessageCog.on_message` early-return on `message.author.bot`, and `on_command_completion` only logs the human-authored trigger message. If you need to inspect what the bot has historically sent (e.g. to debug an `assistant`-role context bug, see the `usage_footer` strip in `gen_reply.py:_get_cleaned_content`), do NOT query `messages.db` — it has no bot rows. Read from Discord directly via `message.channel.history(...)`, which is what `_get_history_message` actually uses to build LLM context.
