...
- A Blackjack hand that reaches five or more cards without busting auto-stands as 過五關. Non-21 過五關 pays a normal win regardless of dealer total. Five-or-more-card 21 keeps the existing main-hand settlement against the dealer plus the extra 1x system-funded bonus. For a VIP, the round's VIP bonus is `max(0.5x of the dealer-paid win, 0.5x of that five-card 21 bonus)` — the larger single 0.5x bonus, not the sum of both; `/casino` only mirrors dealer-paid normal settlement.
...
- The bot account sits at every Blackjack table as a regular player. `games.py` auto-adds it during lobby creation when `bot_user.wallet > 0`; bet sizing, hit/stand/double/split/surrender, and insurance are all decided by `BotPlayerAI` (player_model, 30s timeout, basic-strategy fallback). The bot settles through `settle_blackjack_player` like every human, so its wallet, daily counters, and leaderboard rows behave the same.
- `BotPlayerAI` receives `BotFinancialContext` (lifetime balance / earned / spent + today's casino loss / win / net), `OtherPlayerView`s, and computed Blackjack context with true remaining shoe rank counts, dealer hole card, action risk metrics, and the EV analysis. `cogs/_games/blackjack_ev.py` is a pure, no-LLM engine: from the known dealer hole + up card, the true remaining shoe, H17 rules, and this table's payouts (including the five-card-21 bonus), it computes the exact dealer final-total distribution and per-action expected value, and the EV-max legal action drives the recommendation and the hole-card-aware fallbacks (`fallback_action` with `dealer_cards`/`shoe`, `fallback_insurance` taking insurance only on a known dealer Blackjack); split EV is a flagged estimate. The AI remains the final decision maker when it returns a legal structured action, defaulting to the highest-EV action; fallback only covers timeout, API failure, parse failure, or invalid actions. Decision results carry a `reason` field that surfaces as `💭 ...` on the bot's seat embed. Prompts in `cogs/_games/prompts.py` are English strategy prompts but require Traditional Chinese `reason`; do not turn them back into prescriptive basic-strategy lookup tables.
