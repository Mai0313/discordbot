...
- Per-user long-term memory is markdown under `data/memories/<user_id>/` in two tiers: `main.md` is the small distilled hot tier the reply model reads on demand via the `get_user_memory` tool; `detail.md` is the append-only cold tier keeping consumed and evicted raw entries verbatim, hard-capped at `DETAIL_FILE_MAX_BYTES` with oldest-entry trimming (safe because content past the consolidation read window is unreachable by every consumer; consolidation reads its tail window as provenance, `/memory show` with the `detail` option pages its newest window, and it is never read on the reply path). `raw.md` stages phase-1 output and `main.bak.md` keeps the previous main generation as a recovery point. File-based on purpose and keyed by user id (cross-server, shared with DMs). Do not move it into SQLite or add legacy-layout fallbacks.
...
- `regenerate_main_memory` in `_memory/pipeline.py` is the shared pure-evidence rebuild path for `/memory regenerate` (default `memories_model`, per-user `MEMORY_REGENERATION_COOLDOWN_SECONDS` separate from the consolidation cooldown) and `scripts/regen_memories.py` (caller-specified model/effort/folder, dry-run by default, `--apply` required to rewrite files). It rebuilds main.md from the detail tail window plus unconsumed raw only — never the existing main — via `consolidate(existing_main="", compact=True)` so a window-sized corpus fits the output-token budget; `_rewrite_shrank_too_much` is intentionally skipped (no prior size to compare), failures keep main and raw untouched, and success retires raw like a normal consolidation. The offline script preserves the stored identity line via `store.read_main_identity`.
