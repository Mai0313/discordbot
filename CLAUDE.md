...
- **API surface** (all native async — every function opens an `AsyncSession(bind=_engine, expire_on_commit=False)` directly, so tests can monkeypatch `_engine` and the swap takes effect immediately. There are no `_*_sync` helpers and no `asyncio.to_thread` shim; if you're tempted to add one, you don't need it):
    ...
    - `transfer(sender_id, sender_name, receiver_id, receiver_name, amount)` — atomic conditional debit + credit inside one transaction; rejects self-transfer, non-positive amounts, and insufficient funds
...
- **Bet flow for games**: bets are withdrawn up-front via `place_bet(...)`, then the round resolves and `settle_game(delta=payout)` credits back `bet + delta` (so a regular win pays `2 * bet`, push pays `bet`, loss pays `0`, natural Blackjack pays `int(bet * 2.5)`). Withdrawing first via a conditional DB update is what stops a single user firing two `/dice bet:100` commands in parallel from double-spending the same balance. **After every player settlement, the same round also calls `house_settle(user_id=bot, name=bot, delta=-player_delta)` to mirror the P&L into the dealer's ledger row.** Over-bets are auto-clamped to the player's balance ("auto all-in") inside `place_bet`; only an empty balance rejects the round outright. Both embeds surface a "已自動 all-in" hint when this kicks in so the player notices.
...
- **Shared Blackjack settlement** lives in `cogs/_games/settlement.py`. `settle_blackjack_round(...)` calls the pure `settle()` rule function, credits the gross payout via `settle_game`, mirrors the inverse delta into the house ledger via `house_settle`, and returns `BlackjackSettlement` plus dealer-prompt detail text. Keep this helper as the single DB-settlement path for both initial natural Blackjack hands in `games.py` and interactive hands in `views.py`.
- **`BlackjackView`** (`cogs/_games/views.py`) drives the `Hit` / `Stand` flow. `interaction_check` restricts the buttons to the original `owner_id`; `on_timeout` auto-stands so a walk-away still settles. `_finalize` delegates DB settlement to `settle_blackjack_round(...)` and asks `DealerAI.settle` for one closing line.
...
- **House ledger row.** Every player settlement is mirrored into the bot's own `UserAccount` row via `house_settle(delta=-player_delta)` in both `games.py` (dice) and `settlement.py:settle_blackjack_round` (all Blackjack hands). The bot's row is excluded from `/leaderboard` and surfaced separately by `/house`.
...
- **Bets are withdrawn up-front, payouts credit `bet + delta`.** `settle_blackjack_round(...)` and `GamesCogs.dice` both rely on this — if you ever change `settle()` in `cogs/_games/blackjack.py` to return absolute payout instead of a delta, the call sites will silently double-pay. Tests in `tests/test_blackjack.py` lock down the delta semantics.
- **The dealer is a real `UserAccount` row, with a deliberately negative balance.** `house_settle` shadows every `settle_game` call by writing `-player_delta` to the bot's own `user_id`. Unlike `settle_game`, `house_settle` does *not* clamp at zero — the bot has effectively infinite funds, so its running balance can and should go negative when the casino is losing. `/leaderboard` filters this row out via `top_n(exclude_user_ids=(bot.user.id,))`; `/house` reads it via `get_account` to surface gross flows alongside the (possibly negative) net.
