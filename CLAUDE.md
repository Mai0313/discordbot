...
- **Dragon Gate** (射龍門): two pillar cards make an opening, third card resolves the bet. `gate_win` pays 1:1, `pillar_hit` charges 2× into the pot, `outside_lose` charges 1×. Pair pillars require a `higher`/`lower` choice first; `pair_win` pays 1:1, `pair_pillar_hit` (super-pillar-hit) charges **3× into the pot**, `pair_lose` charges 1×. Every player contributes the ante up front (`player_deltas[user_id] = -ante` at start, settled at table end) so a single `gate_win` for `ante` shows up as "持平" — that is by design.
- **Shared settlement** lives in `cogs/_games/settlement.py`. `settle_wager(...)` is the single DB-settlement path; `settle_blackjack_round(...)` and `settle_dragon_gate_player(...)` wrap it. Used by interactive games and natural-Blackjack early settles.
