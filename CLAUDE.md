...
- The read path does not inject memory: the slow model fetches it on demand via the `get_user_memory` function tool (defined in `cogs/_gen_reply/memory_tool.py`), added to the QA-path `tools` alongside the provider built-ins and driven by a small agentic loop in `_handle_message_reply` (`ResponseStreamer.consume_turn` per turn, `finalize` once; usage summed across turns). The per-request allowlist (`build_memory_allowlist`: conversation authors + `.mentions`, minus the bot) is the permission boundary, surfaced to the model as a `role=system` callable-users block; `resolve_user_memories` enforces it server-side before reading any `main.md`. The when-to-call rule lives in `REPLY_PROMPT` (developer authority), never an input wrapper. SUMMARY (`memory_enabled=False`) and IMAGE paths get no tool. Pass a memory-free, target-centered `message_list` to `schedule_memory_update` so extraction never re-ingests stored memory and distant non-target history is omitted.
