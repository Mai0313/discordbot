...
- The read path is tool-only: normal QA replies expose `read_user_memory(user_id)` with a per-reply allowlist built from the trigger user, trusted visible author prefixes, and explicit Discord user mentions in history/reference/current context. Do not automatically inject `main.md` into the prompt. Pass a memory-free, target-centered `message_list` to `schedule_memory_update` so extraction never re-ingests stored memory and distant non-target history is omitted.
