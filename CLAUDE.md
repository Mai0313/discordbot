...
- Two-phase pipeline in `cogs/_memory/`: fire-and-forget `schedule_memory_update` runs phase-1 extraction after each streamed reply on the QA route (per-user in-flight de-dupe replays only the newest skipped turn); the SUMMARY route calls `_handle_message_reply` with `memory_enabled=False`, skipping both memory injection and extraction. Phase-1 extraction is evidence-dense on purpose (concrete specifics, short verbatim quotes) but stays behind the no-op gate and high-signal bar. Topical interests and likes are gated against over-promotion: phase-1 logs a one-off mention as a dated 近期事件 observation rather than asserting a stable interest, and phase-2 only promotes an interest into the stable sections after it recurs across `INTEREST_PROMOTION_MIN_OCCURRENCES` distinct conversations, demoting or aging out under-evidenced ones; operating preferences the user enforces are exempt. Phase-2 consolidation distills the raw batch plus a `read_detail_tail` window into the main file once `RAW_CONSOLIDATION_THRESHOLD` accumulates and the per-user `MEMORY_CONSOLIDATION_COOLDOWN_SECONDS` elapsed; the `RAW_CONSOLIDATION_MAX_BYTES` byte trigger bypasses the cooldown, and a process-wide semaphore (`MEMORY_GLOBAL_CONCURRENCY`) caps concurrent background updates. Phase-1 uses `RuntimeModelCatalog.extract_model`, phase-2 `RuntimeModelCatalog.memories_model`. Memory must never add latency to the reply path; LLM failures silently keep the previous state.
...
- The read path injects only the trigger user's main file as a lowest-authority `role=assistant` self-note (string content; assistant rejects `input_text` parts) placed right BEFORE the current-message block, never into top-level `instructions` or `role=system`, and never as the trailing message (a trailing assistant message is Claude prefill semantics through LiteLLM). Pass the memory-free `message_list` to `schedule_memory_update` so extraction never re-ingests stored memory.
- The main file starts with `v1` and has no code-side size clamp: past `MAIN_COMPACTION_TRIGGER_CHARS` the next consolidation runs a compaction pass toward `MAIN_COMPACTION_TARGET_CHARS` (LLM summarization, never truncation; the real bound is the rewrite's output-token ceiling, not the input window). Output guards protect the rewrite: both memory calls set `MEMORY_MAX_OUTPUT_TOKENS`, an `incomplete` response status is refused, and `_rewrite_shrank_too_much` rejects drastic surprise shrinks, keeping raw for retry so a truncated or lossy rewrite never overwrites good memory or its backup. `/memory show` paginates oversized memory with prev/next buttons. The consolidation input carries a `today:` line so the dated `## 近期脈絡` section can age out. Memory is always on (no env switch; the `memory_enabled` function parameter still gates per call) and `/memory clear` always deletes.
- `regenerate_main_memory` in `_memory/pipeline.py` is the shared pure-evidence rebuild path for `/memory regenerate` (default `memories_model`, per-user `MEMORY_REGENERATION_COOLDOWN_SECONDS` separate from the consolidation cooldown) and `scripts/regen_memories.py` (caller-specified model, hardcoded `__main__` args, run from repo root). It rebuilds main.md from the detail tail window plus unconsumed raw only — never the existing main — via `consolidate(existing_main="", compact=True)` so a window-sized corpus fits the output-token budget; `_rewrite_shrank_too_much` is intentionally skipped (no prior size to compare), failures keep main and raw untouched, and success retires raw like a normal consolidation. The offline script preserves the stored identity line via `store.read_main_identity`.
