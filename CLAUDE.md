...
- **`BlackjackView`** (`cogs/_games/views.py`) drives the `Hit` / `Stand` flow. `interaction_check` restricts the buttons to the original `owner_id`; `on_timeout` auto-stands so a walk-away still settles. `_finalize` calls `settle()` for the outcome, credits `bet + delta` via `settle_game`, mirrors the inverse delta into the house ledger via `house_settle`, and asks `DealerAI.settle` for one closing line.
- **Dealer identity is dynamic.** `GamesCogs._dealer_identity()` returns `(bot.user.id, bot.user.display_name)`; the embeds use that name for the description prefix, the field titles, and the house-ledger row, so message history stored by `log_msg.py` shows dealer banter under the bot's own display name (which is what `gen_reply.py` then sees as its own past output).
- **House ledger row.** Every player settlement is mirrored into the bot's own `UserAccount` row via `house_settle(delta=-player_delta)` in both `games.py` (dice / dealt-Blackjack natural) and `views.py:_finalize` (player-driven Blackjack hands). The bot's row is excluded from `/leaderboard` and surfaced separately by `/house`.
