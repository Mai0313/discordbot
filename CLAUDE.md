...
- The bot also keeps per-server (community) memory at `data/memories/<bot_id>/<server_id>/` through that shared engine with `_memory/server_prompts.py` (sections `伺服器輪廓 / 社群文化 / 常見話題 / 重要事實 / 近期脈絡`, same `v1` contract; community-level only, never an individual's personal facts). Unlike user memory it needs no selection phase / allowlist / function tool: `gen_reply` reads the current guild's `main.md` directly and injects `render_server_memory_block(...)` as a low-authority `role=assistant` block (zero extra LLM latency), and schedules a server-scope `schedule_memory_update` (subject `target_server_id:`, the whole conversation, no target-centering) after each guild QA reply. DMs and the SUMMARY route skip both. The write side is also gated by `_source_channel_is_public`: a channel `@everyone` cannot view (private / mod channels, private threads) never feeds the server-wide memory that any member can read via `/memory server show`; the read injection is left open since the memory only holds public-channel content.
