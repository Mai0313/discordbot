...
- **AI Blackjack dealer phase**: `BlackjackLobbyView` creates rounds with `auto_play_dealer=False`; after every player stands/busts, `BlackjackView._play_dealer_locked()` calls `DealerAI.decide_blackjack_action()` via `client.responses.parse(text_format=BlackjackDealerDecision)`. Guard rails force obvious actions (≤11 hit, 21 stand), fallback to the old basic rule on timeout/parse failure, and show a compact decision path in the final embed.
