...
- Runtime model strings live in `RuntimeModelCatalog` in `src/discordbot/typings/models.py`; keep `slow_model`'s peak-hour dispatch (both arms dispatch the same model today; the split is kept on purpose because Gemini Pro has historically slowed down at peak). Routing runs on `route_model` and the phase-1 `get_user_memory` selection on `tool_model` (both flash-tier, off the `slow_model` tier): lightweight critical-path decisions before the answer, on flash rather than flash-lite because effort grading and nickname→id matching need the extra language skill. Provider selection happens through LiteLLM via model strings and `extra_body`.
- Streaming SDK objects are named `responses`; loop items are named `response`.
- AI progress is communicated with reactions on the user's message, scheduled through `utils.reactions.ReactionStatusChain` (ordered, fire-and-forget, flushed at pipeline end); the QA route shows 💭. While the answer model thinks, `ResponseStreamer` previews the reasoning summary inside the single reply message as `-#` subtext (mention-escaped, newest-tail window) and the first content delta replaces it; there are still no separate status messages. Discord edits run on a time-based snapshot editor task (`preview_interval_seconds`, stop-event shutdown so an in-flight write always lands before the final edit) so stream consumption never waits on Discord.
- Attachment ingestion is gated by `get_supported_modalities` for the slow model; unsupported attachments are dropped before any LLM call. Images are downscaled to a 3072px longest edge and JPEG re-encoded via `utils.images.shrink_image_bytes` (alpha and palette images keep PNG, GIF and undecodable payloads pass through) into `input_image` data URIs; other supported attachments are sent as `input_file`. Rendered attachment parts are LRU-cached per message in `MessageInputBuilder`, keyed on message id, edit time, and source counts; failed renders are not cached so the next reply retries.
- For Discord embeds, prefer `media.discordapp.net` `proxy_url` over origin URLs because CDN links expire.
- Reference and current messages are processed once (`_get_reference_and_current`) and shared by routing and the answer. The QA context — history (fetched once as `RenderedHistory`, whose raw side feeds the memory allowlist), server memory, and the memory selection request (`_prepare_reply_context` → `ReplyContext`) — builds speculatively in one task concurrent with the route call; non-QA routes cancel it and SUMMARY rebuilds at history 100 with `memory_enabled=False`, reusing the shared parts. The route returns `RouteDecision` (`decision` plus a graded `effort`); QA and SUMMARY override the slow model's reasoning effort with it and the usage footer shows `model (effort)`. Route and memory-selection inputs are text-only (`strip_attachment_parts` swaps payload parts for `[attachment: ...]` markers); the answer request keeps the full parts. Memory selection is bounded by `MEMORY_SELECT_TIMEOUT_SECONDS` and degrades to a no-memory answer; `_handle_video_reply` polling is bounded by `VIDEO_GENERATION_TIMEOUT_SECONDS`. Multi-message fetches still use `asyncio.gather` with the task list built in a `for` loop on its own lines. Logfire spans (`gen_reply pipeline` / `route` / `context build` / `memory selection` / `answer`) plus first-reasoning-delta and first-content-delta logs quantify per-phase latency.
...
- Instruction hierarchy (OpenAI model spec): `system` > `developer` > `user` > `assistant`. `system` is the highest; `developer` carries the app's rules like a function definition; `user` carries end-user input like its arguments; `assistant` is model output and carries the lowest authority. The top-level `instructions` parameter is equivalent to a `developer` message and outranks everything in `input`, but does not persist across turns under conversation state.
- Separator messages use `role=system`, not `developer`, for Gemini and Claude compatibility through LiteLLM.
- Gemini (via LiteLLM) cannot use a custom function tool and its built-in tools (`googleSearch` / `urlContext`) in the same Responses request: sending both **silently drops the built-in grounding** (search annotations go to zero) — it does not 400, so a probe that only checks "the call succeeds / the function fires" will miss it. When mixing tool kinds, verify the built-in grounding still fires, not just that there is no error. This is why memory retrieval is two-phase: a selection request carries only the function tool, and the answer request carries only the built-in tools (see the Memory section).
- Gemini may prepend leading newlines to streamed reasoning and content output; `ResponseStreamer._consume` strips them on the first delta of each. Gemini thought summaries only flow through the Responses API.
...
- Money inputs are string `SlashOption`s parsed by the bot, not integer options; the shared normalizer is `utils/amount_parsing.parse_decimal_amount`, wrapped by `_parse_positive_amount` / `_parse_collect_amount` (economy), `parse_wager_amount` (games), and `parse_bait_quantity` (fishing). Malformed text returns `build_invalid_amount_embed` ephemerally before any mutation. Prefer this pattern for any new numeric input that can exceed Discord limits.
...
- Central banker access (`UserAccount.is_central_banker`, set offline via direct DB update) is separate from Discord admin. Keep `ECONOMY_ALLOW_CENTRAL_BANK_SELF_APPROVAL` unset or `false` in production.
...
- `stock_profile` is the source of truth for virtual companies; maintain it offline via direct DB edits. Do not seed companies from runtime code and do not add legacy schema migrations.
...
- Global anti-inflation guardrails are constants in `cogs/_stock/market.py` (`MARKET_VOLATILITY_SCALE_BPS`, `GLOBAL_MAX_TICK_CHANGE_BPS`, Taiwan-style `apply_daily_price_limit`); re-measure market behavior with an offline simulation before changing them.
...
- Per-user long-term memory is markdown under `data/memories/<user_id>/` in two tiers: `main.md` is the small distilled hot tier the reply model reads on demand via the `get_user_memory` tool; `detail.md` is the append-only cold tier keeping consumed and evicted raw entries verbatim, hard-capped at `DETAIL_FILE_MAX_BYTES` with oldest-entry trimming (safe because content past the consolidation read window is unreachable by every consumer; consolidation and regeneration read its tail window as provenance, it has no user-facing view, and it is never read on the reply path). `raw.md` stages phase-1 output and `main.bak.md` keeps the previous main generation as a recovery point. File-based on purpose and keyed by user id (cross-server, shared with DMs). Do not move it into SQLite or add legacy-layout fallbacks.
- Two-phase pipeline in `cogs/_memory/`: fire-and-forget `schedule_memory_update` runs phase-1 extraction after each streamed reply on the QA route (per-user in-flight de-dupe replays only the newest skipped turn); the SUMMARY route calls `_handle_message_reply` with `memory_enabled=False`, skipping both the memory tool and extraction. Phase-1 emits structured `MemoryObservation`s, then a phase-1.5 evaluator reruns strict quality review before raw append; deterministic gates reject casual mentions, hypotheticals, bot suggestions, other-user context, weak confidence, non-target observations, and duplicate `normalized_key`s already present in raw/detail evidence. Phase-2 consolidation distills the raw batch plus a `read_detail_tail` window into the main file once `RAW_CONSOLIDATION_THRESHOLD` accumulates and the per-user `MEMORY_CONSOLIDATION_COOLDOWN_SECONDS` elapsed; the `RAW_CONSOLIDATION_MAX_BYTES` byte trigger bypasses the cooldown, and a process-wide semaphore (`MEMORY_GLOBAL_CONCURRENCY`) caps concurrent background updates. Phase-1 uses `RuntimeModelCatalog.extract_model`, the evaluator uses `RuntimeModelCatalog.memory_evaluator_model`, and phase-2 uses `RuntimeModelCatalog.memories_model`. Memory must never add latency to the reply path; LLM failures silently keep the previous state.
...
- The read path is two-phase because Gemini cannot use a custom function tool together with its built-in search/url tools in one request (verified: mixing them silently drops grounding): phase 1 (`_select_user_memories`) runs an isolated request offering only the `get_user_memory` tool (defined in `cogs/_gen_reply/memory_tool.py`) so the model picks whose memory to read — the current guild's server memory rides in front as background context so a spoken nickname can be resolved to its id — and phase 2 streams the answer (`ResponseStreamer.stream`) with the built-in tools always available and the selected memory injected as a low-authority `role=assistant` context block (`render_memory_context_block`). The per-request allowlist (`build_memory_allowlist`: conversation authors + `.mentions`, minus the bot) is the permission boundary, surfaced to the selection model as a `role=system` callable-users block; `resolve_user_memories` enforces it server-side before reading any `main.md`. In every guild channel a participant already in the allowlist gains the server memory's `## 成員稱呼` nickname-table row as a label suffix (`widen_allowlist_with_aliases`) so the selection model matches community aliases on one line instead of joining across context blocks; this enrichment grants no new access. Only in public channels (`include_absent=_source_channel_is_public`) is the allowlist also widened with absent members' ids, because the nickname table is public but the personal memory it would unlock is not; private channels and DMs keep the conversation-only boundary. The when-to-call rule lives in `MEMORY_SELECT_PROMPT` (developer authority), never an input wrapper. SUMMARY (`memory_enabled=False`) and IMAGE paths run no selection phase. Pass a memory-free, target-centered `message_list` to `schedule_memory_update` so extraction never re-ingests stored memory and distant non-target history is omitted.
- The main file starts with `v1` and has no code-side size clamp: past `MAIN_COMPACTION_TRIGGER_CHARS` the next consolidation runs a compaction pass toward `MAIN_COMPACTION_TARGET_CHARS` (LLM summarization, never truncation; the real bound is the rewrite's output-token ceiling, not the input window). Output guards protect the rewrite: all memory LLM calls set `MEMORY_MAX_OUTPUT_TOKENS`, an `incomplete` response status is refused, and `_rewrite_shrank_too_much` rejects drastic surprise shrinks, keeping raw for retry so a truncated or lossy rewrite never overwrites good memory or its backup. `/memory show` paginates oversized memory with prev/next buttons. The consolidation input carries a `today:` line so the dated `## 近期脈絡` section can age out. Memory is always on (no env switch; the `memory_enabled` function parameter still gates per call).
- `regenerate_main_memory` in `_memory/pipeline.py` is the shared pure-evidence rebuild path for `/memory regenerate` (default `memories_model`, per-user `MEMORY_REGENERATION_COOLDOWN_SECONDS` separate from the consolidation cooldown) and `scripts/regen_memories.py`. The command does not run it inline: `schedule_memory_regeneration` dispatches it as a background task (own `_regeneration_tasks` slot with in-flight de-dupe, mirroring `schedule_memory_update`) and replies "已排程重建" immediately, so the rewrite never blocks the interaction. (caller-specified model/effort/folder, dry-run by default, `--apply` required to rewrite files). It rebuilds main.md from the detail tail window plus unconsumed raw only — never the existing main — via `consolidate(existing_main="", compact=True)` so a window-sized corpus fits the output-token budget; `_rewrite_shrank_too_much` is intentionally skipped (no prior size to compare), failures keep main and raw untouched, and success retires raw like a normal consolidation. The offline script preserves the stored identity line via `store.read_main_identity`.
...
- Only `main.md` carries a single-line author identity (after the `v1` header) for human inspection; read helpers strip it so prompt injection, consolidation input, and `/memory show` never see it. Raw and detail entry headers are timestamp-only.
- `store.py` and `pipeline.py` are keyed by an opaque `scope: str` that is both the relative subdir under `data/memories/` and the registry key; `user_scope(user_id)` (`<user_id>`) and `server_scope(bot_id, server_id)` (`<bot_id>/<server_id>`) build it, and one store + one pipeline serve both flavors (the `MEMORY_GLOBAL_CONCURRENCY` semaphore is shared; in-flight de-dupe and cooldowns stay per-scope). `MemoryExtractorAI` carries its phase-1 / evaluator / phase-2 prompts as fields and `extract(subject, transcript)` takes a per-call subject (`target_user_id:` / `target_server_id:`), so the same engine drives a different flavor by swapping prompts.
- The bot also keeps per-server (community) memory at `data/memories/<bot_id>/<server_id>/` through that shared engine with `_memory/server_prompts.py` (sections `伺服器輪廓 / 社群文化 / 常見話題 / 重要事實 / 成員稱呼 / 近期脈絡`, same `v1` contract; community-level only, never an individual's personal facts). `gen_reply` reads the current guild's `main.md` once (`_read_server_memory`) and injects `render_server_memory_block(...)` as a low-authority `role=assistant` block into both the user-memory selection request and the answer (zero extra LLM latency), and schedules a server-scope `schedule_memory_update` (subject `target_server_id:`, the whole conversation, no target-centering) after each guild QA reply. The `## 成員稱呼` section is a member nickname↔id lookup table — the sole carve-out from the no-individuals rule, recorded as community vocabulary (`stable_fact`, not a personal fact) so the shared extraction gate accepts it; it primes the selection model to resolve aliases, enriches present participants' labels in every guild channel, and only in public channels widens the user-memory allowlist with absent members' ids. DMs and the SUMMARY route skip both. The write side is also gated by `_source_channel_is_public`: a channel `@everyone` cannot view (private / mod channels, private threads) never feeds the server-wide memory that any member can read via `/memory server show`; the read injection is left open since the memory only holds public-channel content.
- User-facing commands are read/rebuild only: `/memory show` and `/memory regenerate` (per-user) and `/memory server show` (guild-only, current guild's community memory). There is no `/memory clear`; the store-level `clear_memory` / `mark_cleared` / `cleared_since` primitives stay for the in-flight abort guards and offline maintenance.
