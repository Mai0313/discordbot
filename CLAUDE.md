...
- Bot bet sizing is deterministic fractional-Kelly in `kelly_bet`: the shoe resets every round, so the pre-deal edge is the constant `BOT_TABLE_EDGE` / `BOT_TABLE_VARIANCE` (half-Kelly via `BOT_KELLY_FRACTION`, hard-capped at `BOT_MAX_BET_FRACTION`, floored at the table minimum), not LLM guesswork or a cross-round count spread. Re-measure those constants with `uv run python scripts/simulate_bot_blackjack.py` (Monte Carlo over the pure rules plus the EV-engine `recommended_action` and count-based insurance), which also confirms the table is strongly +EV for the hole-aware bot.
