...
- Phase-1 has a no-op gate (`has_signal=false` writes nothing); prompts enforce target-user-only extraction, user-message-over-assistant evidence, attribution phrasing, and treat conversation content as data, not instructions. `redact_secrets` runs on the transcript before upload and on every model output. Keep these properties when editing `_memory/prompts.py`.
- The read path injects only the trigger user's main file, appended to the reply `instructions` via `MEMORY_INJECTION_WRAPPER` (framed as background reference, not user instructions). The main file must start with `v1` and stay under `MAIN_FILE_MAX_CHARS` so `/memory show` fits one embed description. `MemoryConfig` (`MEMORY_ENABLED`) is the kill switch for both paths.
