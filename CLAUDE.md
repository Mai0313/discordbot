...
- Fishing is a single-player money-sink mini-game, not a casino-vs-player game. It follows the `/stock` single-public-message edit-in-place pattern (`cogs/_games/fishing_views.py`: `FishingPublicView` base, owner-only `interaction_check`, 180s idle `on_timeout` delete, `edit_fishing_message` dispatcher), not the multiplayer lobby. The `/games fishing` subcommand defers, builds a `FishingContext`, and calls `open_fishing_panel`.
- This PR is text/emoji only: no Pillow fish art or dex grid PNG. Catch results, 魚簍, 圖鑑, and leaderboards are text embeds. A PNG leaderboard board (reusing `_economy/boards.py`) is a deliberate follow-up.
- Anti-inflation invariant: every `(rod, bait)` loadout has a negative per-cast EV. Pure rules live in `cogs/_games/fishing.py` (rng-injectable: `roll_rarity`/`select_species`/`roll_size`/`sell_value`/`cast_fish`), the catalog and tuning in `typings/fishing.py`. `per_cast_ev` is asserted negative for every loadout in `tests/test_fishing.py`, and `scripts/simulate_fishing.py` confirms the empirical mean. Re-balance the catalog there, never inline.
- Economy split: rod/bait purchases burn 虛擬歡樂豆 via `apply_ordered_wallet_deltas` (a permanent sink → `total_spent`); fish sales credit via `credit_with_repayment` (faucet → `total_earned`); the wallet invariant `balance == total_earned - total_spent` is preserved by always routing through those helpers. `execute_cast` touches only `data/fishing.db` and is fully atomic — no money moves at cast time, which is why catches are not auto-sold. Cross-file writes are not atomic together: buy debits economy then grants (refunds on grant failure); sell marks rows sold then credits (un-sells on credit failure); the residual hard-crash window mirrors the casino-ledger split-file note.
- Persistence is `data/fishing.db` (`cogs/_games/fishing_database.py`, own module-level `_engine`): one equipped rod per user with durability that breaks at 0, per-type bait counts, and an append-only `fishing_catch` log kept after sale so the 圖鑑 survives selling. Money columns use `StoredInteger`; physically bounded counters (durability, size, bait counts) use `Integer` so SQL `MAX`/`ORDER BY` stay correct. No migrations; offline maintenance via `scripts/manage_fishing.py`.
- Fishing does not touch `casino_account` daily counters or `casino_ledger`, and the VIP multiplier does not affect fishing payouts in v1. Narrator banter (`SystemNarrator.catch_fish`, `end_user_id="casino_fish_catch"`) is optional and runs as a revision-guarded background refresh; the deterministic per-rarity fallback line (`prompts.py::fishing_catch_fallback_line`) shows immediately so a cast never waits on the LLM.
