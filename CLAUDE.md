...
- Only `main.md` carries a single-line author identity (after the `v1` header) for human inspection; read helpers strip it so prompt injection, consolidation input, and `/memory show` never see it. Raw and detail entry headers are timestamp-only; `append_detail` and the raw / detail read paths still strip legacy ` | <identity>` suffixes from files written before the removal.
