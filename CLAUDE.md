...
- AI progress is communicated with reactions on the user's message, scheduled through `utils.reactions.ReactionStatusChain` (ordered, fire-and-forget, flushed at pipeline end). Preserve the no-intermediate-message UX.
...
- Reference and current messages are processed once (`_get_reference_and_current`) and shared by routing and the answer. The QA context — history, server memory, and the memory selection request (`_prepare_reply_context` → `ReplyContext`) — builds speculatively in one task concurrent with the route call; non-QA routes cancel it and SUMMARY rebuilds at history 100 with `memory_enabled=False`, reusing the shared parts. Multi-message fetches still use `asyncio.gather` with the task list built in a `for` loop on its own lines. Logfire spans (`gen_reply pipeline` / `route` / `context build` / `memory selection` / `answer`) plus a first-content-delta log quantify per-phase latency.
