...
- **AI 聊天**：在 server 标记机器人或发送 DM。它可以回答问题、总结近期聊天、检查支持的附件、生成或编辑图片、生成短视频、在 thread 延续长回复，并在可用时使用 model-provided web tools。
...
- **模拟股市**：`/stock` 开启一则公开 market message，内含多家 virtual company；选股后会开 private detail panel 处理交易、仓位摘要、近期交易记录、定期刷新新闻与 7 日图表，只有开启面板的 user 可以操作 controls。
...
| `/balance`                                                       | 私密显示你的虚拟欢乐豆余额、债务、净资产与 VIP 状态。                |
| `/checkin`                                                       | 领取每日签到奖励。                                                   |
| `/vip`                                                           | 购买永久 VIP 权益。                                                  |
...
| `/stock`                                                         | 公开股票市场消息，明细、交易、新闻、记录走 private panel。           |
...
- `stock.db`：模拟 stock profile、price tick、position、trade operation、ordered trade leg 与 AI-or-fallback stock news。
