...
赌场游戏的 final response embed 会在回合结算三分钟后自动删除，余额不足而拒绝开局的回复也会在三分钟后清掉，避免频道被小游戏刷屏。
...
- `/house` — 查看庄家在 `/dice` 与 `/blackjack` 累积的输赢。庄家资金无上限，所以 ledger balance 可以是负数（代表整体玩家从庄家手里赢走的点数比较多）。
